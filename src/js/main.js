...
function setHomeSize() {
  ...
  home.style.height = `${
    windowHeight > MAX_HEIGHT ? MAX_HEIGHT : windowHeight
  }px`;
}
...
function scrollToUtil(target = 0) {
  ...
  const cancel = () => (
    clearTimeout(timeout), window.cancelAnimationFrame(scroll)
  );
...
const randomizer = () =>
  (Math.random() * 15 + 10) * (Math.random() > 0.5 ? 1 : -1);
...
window.addEventListener(
  'scroll',
  debouncer(() => points.forEach(c => backgroundChange(c)), 20)
);
...
function displayBackTop() {
  ...
  setTimeout(
    () => (backTop.style.display = shouldHide ? 'none' : 'block'),
    500
  );
}
